...
const getProducts = async (req, res) => {
	try {
		const products = await Product.findAll({
			where: {
				status: 'pending', // Retrieve only pending products
			},
		});
...
const submitProduct = async (req, res) => {
	try {
		...
		// Log the uploaded file and image filename
		console.log('Uploaded file:', req.file);
		console.log('Product image:', productImage);

		const newProduct = await Product.create({
			productName, // Updated property name
			affiliateLink, // Updated property name
			image: productImage, // Updated property name
			status: 'pending',
		});
...
const publishProduct = async (req, res) => {
	try {
      ...
      `;
		...
		const sentMessage = await telegramBot.sendMessage(chatId, message);
...
const deleteProduct = async (req, res) => {
	try {
		...
		await telegramBot.deleteMessage(chatId, product.telegramMessageId); // Delete message from Telegram
